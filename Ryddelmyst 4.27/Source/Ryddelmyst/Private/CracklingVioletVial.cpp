...
void UCracklingVioletVial::OnEquip_Implementation(AActor* EquippedActor)
{
	...
	if (Character)
	{
		...
		Character->GetMetamagicMap()[ARyddelmystCharacter::ID_SPELL_ELECTRICSNOWBALL][UCracklingVioletVial::ID][ARyddelmystCharacter::ID_SPELL_ASPECT_CONJURATION][ARyddelmystCharacter::ID_METAMAGIC_CATEGORY_CREATION] = [=](ARyddelmystCharacter* ConjuringCharacter) -> std::vector<AElectricSnowball*>
		{
			std::vector<AElectricSnowball*> ResultArray;
		...
		Character->GetMetamagicMap()[ARyddelmystCharacter::ID_SPELL_ELECTRICSNOWBALL][UCracklingVioletVial::ID][ARyddelmystCharacter::ID_SPELL_ASPECT_TRANSMUTATION][ARyddelmystCharacter::ID_METAMAGIC_CATEGORY_SPAWN] = [=](ARyddelmystCharacter* TransmutingCharacter, const FTransform& SpawnTransform, const FVector& LaunchDirection, const std::vector<AElectricSnowball*>& Bullets)
		{
			...
			for (auto Bullet : Bullets)
			{
				...
				Bullet->Cast(TransmutingCharacter, DistRotation.RotateVector(LaunchDirection));
			}
		};
	}
}