...
void AElectricSnowball::OnHit(UPrimitiveComponent* HitComp, AActor* OtherActor, UPrimitiveComponent* OtherComp, FVector NormalImpulse, const FHitResult& Hit)
{
    ...
    {
        // todo: check that OtherActor implements the IStats interface and iff he does then pull down the stats for modification by ShockedStatusEffect (modifications which will be removed when the shocked effect ends)
        ...
        StatusEffect->SetDuration(ShockDuration);
